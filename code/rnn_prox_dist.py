...
CF_ie = 1.0 # connection fraction E->I
...
r_target_e = 0.1 # homeostatic excitatory target firing rate
r_target_i = 0.1 # homeostatic inhibitory target firing rate
mu_IP = 0.001 # threshold adaption rate
T_e_max_init = 1.
T_i_max_init = 1.
mu_mem_noise = 0.
sigm_mem_noise = np.sqrt(0.04)
##

## Synaptic Normalization
w_total_ee = 1. # total presynaptic E->E input
...
## Simulation
n_t = 5000 # simulation time steps
##
...
def main():
	...
	W_ee_rec = np.ndarray((n_t,N_e,N_e))
	W_ei_rec = np.ndarray((n_t,N_e,N_i))
	...
	for t in tqdm(range(n_t)):
		...
		## Update tresholds
		T_e = update_th(T_e,x_e,r_target_e,mu_IP)
		T_i = update_th(T_i,x_i,r_target_i,mu_IP)
		##
		
		## Update weights
		W_ee = hebb_ee(W_ee,x_e,x_e_old,mu_plast_ee)
		W_ei = hebb_ei(W_ei,x_e,x_i_old,r_target_e,mu_plast_ei)
